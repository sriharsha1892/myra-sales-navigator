...
import type { SortField, CompanyEnriched, ResultSource } from "@/lib/types";
          ...
          {viewMode === "companies" && (
            <>
              ...
              <div className="inline-flex rounded-input border border-surface-3 bg-surface-2 p-0.5">
                {(["icp_score", "name", "employee_count"] as SortField[]).map((field) => {
                  ...
                  return (
                    <button
                      ...
                      className={`rounded-[6px] px-2 py-0.5 text-[11px] font-medium transition-all duration-200 ${
                        isActive
              ...
              <button
                ...
                className="btn-press flex h-6 w-6 items-center justify-center rounded-input border border-surface-3 bg-surface-2 text-xs text-text-tertiary hover:text-text-primary"
                title={sortDirection === "desc" ? "Descending" : "Ascending"}
              >
                {sortDirection === "desc" ? "\u2193" : "\u2191"}
              </button>
              {/* Grouping pill group */}
              <span className="text-[10px] text-text-tertiary">Group</span>
              <div className="inline-flex rounded-input border border-surface-3 bg-surface-2 p-0.5">
                {(["icp_tier", "source", "none"] as const).map((g) => {
...
function ContactsView({
  ...
  groupedContacts: { domain: string; companyName: string; icpScore: number; contacts: import("@/lib/types").Contact[] }[];
  personaGroups: import("@/hooks/useContactsTab").PersonaGroup[];
  fetchedCount: number;
  totalCount: number;
...
function DemoResults({ onDismiss }: { onDismiss: () => void }) {
  const [mockCompanies, setMockCompanies] = useState<import("@/lib/types").CompanyEnriched[]>([]);
