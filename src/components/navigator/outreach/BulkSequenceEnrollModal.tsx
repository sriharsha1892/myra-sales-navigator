...
interface BulkSequenceEnrollModalProps {
  contacts: Array<{ contactId: string; companyDomain: string }>;
  onClose: () => void;
}
...
export function BulkSequenceEnrollModal({
  contacts,
  onClose,
}: BulkSequenceEnrollModalProps) {
  ...
  const handleEnroll = useCallback(async () => {
    ...
    setEnrolling(true);
    try {
      const res = await fetch("/api/outreach/enrollments/bulk", {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.error || "Bulk enrollment failed");
      }
      ...
      if (data.skipped > 0) parts.push(`${data.skipped} already in sequence`);
      if (data.errors?.length > 0) parts.push(`${data.errors.length} batch errors`);

      addToast({
        message: parts.join(", "),
        type: data.enrolled > 0 ? "success" : "warning",
      });
  ...
  }, [selectedId, contacts, addToast, queryClient]);
    ...
    <Overlay open={true} onClose={onClose} backdrop="blur" placement="center">
      <div className="w-full max-w-[560px] rounded-card border border-surface-3 bg-surface-1 shadow-2xl">
        ...
        <div className="max-h-[60vh] overflow-y-auto px-5 py-4">
          ...
          {result && (
            <div className="mb-4 rounded-input border border-surface-3 bg-surface-0 px-4 py-3">
              <p className="text-xs font-medium text-text-primary">Enrollment complete</p>
              <div className="mt-1.5 space-y-0.5 text-[10px] text-text-secondary">
                {result.enrolled > 0 && (
                  <p className="text-green-400">{result.enrolled} contact{result.enrolled !== 1 ? "s" : ""} enrolled</p>
                )}
                {result.skipped > 0 && (
                  <p className="text-text-tertiary">{result.skipped} already in this sequence (skipped)</p>
                )}
                {result.errors.length > 0 && (
                  <p className="text-danger">{result.errors.length} batch error{result.errors.length !== 1 ? "s" : ""}</p>
                )}
              </div>
            </div>
          )}
          ...
          <div className="flex items-center justify-end gap-2 border-t border-surface-3 px-5 py-3">
            ...
            {!result && (
              ...
              >
                {enrolling
                  ? "Enrolling..."
                  : `Enroll ${contacts.length} Contact${contacts.length > 1 ? "s" : ""}`}
              </button>
            )}
          </div>
        )}
