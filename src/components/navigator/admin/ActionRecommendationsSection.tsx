...
export function ActionRecommendationsSection() {
  ...
  const toggleGlobal = () => {
    updateAdminConfig({ actionRecommendationEnabled: !enabled });
  };
  ...
  const toggleRule = (id: string) => {
    ...
    updateAdminConfig({ actionRecommendationRules: updated });
  };
  ...
  const moveRule = (index: number, direction: "up" | "down") => {
    ...
    updateAdminConfig({ actionRecommendationRules: arr });
  };
  ...
  return (
    <div className="rounded-card border border-surface-3 bg-surface-1 p-5">
      ...
      <p className="mt-1 text-xs text-text-tertiary">
        Show recommended next action in company dossier. Toggle and reorder rules.
      </p>
